...
namespace Vella.Common
{
    ...
    public static class NativeDebug
    {
        ...
        private static void OnCompilationStarted(object obj)
        {
            ...
            NativeDebugSharedData.Stream.Dispose();
        }
        ...
        private static void SceneViewOnDuringSceneGui(SceneView obj)
        {
            ...
            NativeDebugSharedData.State.IsVisible = obj.rootVisualElement.visible;

            if (NativeDebugSharedData.State.IsTransitioning)
                return;

            if (!NativeDebugSharedData.Stream.IsNextStreamReady)
                NativeDebugSharedData.Stream.AllocateNext();

            if (!NativeDebugSharedData.Stream.IsCurrentStreamReady)
                return;

            if (NativeDebugSharedData.Stream.Reader.ComputeItemCount() == 0)
                return;
            ...
            if (NativeDebugSharedData.Time.FrameCount - NativeDebugSharedData.Time.LastFrame > 1)
            {
        ...
        public static void QueueDrawing<T>(T drawing) where T : struct, INativeDebuggable
        {
            ...
            if (!NativeDebugSharedData.State.IsVisible)
                return;
            ...
            var count = NativeDebugSharedData.Stream.Reader.ComputeItemCount();
        ...
        /// <param name="size">size of the cube (extents*2 or max-min)</param>
        /// <param name="color">the color of the cube lines</param>
        [Conditional("UNITY_EDITOR"), BurstDiscard]
        ...
        public static void Log(NativeString512 text)
        {
    ...
    public struct Polygon : INativeDebuggable
    {
        public DebugDrawingType Type;
        public Color Color;
    ...
    public static class NativeDebugSharedData
    {
        ...
        private struct Container
        {
            ...
            public StreamRotation StreamRotation;
        ...
        public static ref CameraData Camera => ref SharedData.Data.CameraData;

        public static ref StreamRotation Stream => ref SharedData.Data.StreamRotation;

        public static ref StateData State => ref SharedData.Data.StateData;

    }

    public unsafe struct StreamRotation
    {
        ...
        public bool IsNextStreamReady;
        public bool IsCurrentStreamReady;

        /// <summary>
        /// Rotates from using the current stream to using a new pre-allocated stream.
        /// </summary>
        public void UseNext()
        {
            if (IsNextStreamReady)
            {
                Last = Current;
                Current = Next;
                Writer = Current.AsWriter();
                Reader = Current.AsReader();
                IsCurrentStreamReady = true;
                IsNextStreamReady = false;
                IsCurrentStreamReady = true;
            }
        }

        private static long GetBlockAddr(ref NativeStream stream)
        {
            long* addr = (long*)UnsafeUtility.AddressOf(ref stream);
            if (addr != null && *addr > 0)
            {
                return *addr;
            }
        ...
        /// <summary>
        /// Allocates the next stream from managed, so that its ready to be used from within burst code.
        /// </summary>
        public void AllocateNext()
        {
            if (!IsNextStreamReady)
            {
                var newStream = new NativeStream(MaxCount, Allocator.Persistent);
                Next = newStream;

                NativeDebugAllocationTracker.Allocations.Add(newStream);

                var last = Last;
                IsNextStreamReady = true;

                var addr = GetBlockAddr(ref last);
                if (addr != 0)
                {
                    if (NativeDebugAllocationTracker.Allocations.Contains(addr))
                    {
                        last.Dispose();
                        NativeDebugAllocationTracker.Allocations.Remove(addr);
                    }
                }
            }
        }
        ...
        public void Dispose()
        {
            IsCurrentStreamReady = false;

            var copy = new NativeDebugAllocationTracker.TrackedAllocation[NativeDebugAllocationTracker.Allocations.Count];
            NativeDebugAllocationTracker.Allocations.CopyTo(copy);
            foreach (var allocation in copy)
            {
                allocation.Stream.Dispose();
                NativeDebugAllocationTracker.Allocations.Remove(allocation);
            }
    ...
    public unsafe class NativeDebugAllocationTracker
    {
        public static TrackedAllocationHashSet Allocations = new TrackedAllocationHashSet();

        public class TrackedAllocationHashSet : HashSet<TrackedAllocation>
        {
            public bool Contains(long ptr) => Contains(new TrackedAllocation { BlockPtr = ptr });

            public bool Remove(long ptr) => Remove(new TrackedAllocation { BlockPtr = ptr });
        }

        public struct TrackedAllocation : IEquatable<TrackedAllocation>, IEquatable<long>
        {
            public long BlockPtr;
            public NativeStream Stream;

            public static implicit operator TrackedAllocation((long, NativeStream) input)
                => new TrackedAllocation { BlockPtr = input.Item1, Stream = input.Item2 };

            public static implicit operator TrackedAllocation(NativeStream input)
                => new TrackedAllocation { BlockPtr = GetBlockAddr(ref input), Stream = input };
            ...
            public bool Equals(TrackedAllocation other) => BlockPtr == other.BlockPtr;
            public bool Equals(long other) => BlockPtr == other;
            public override bool Equals(object obj) => obj is TrackedAllocation other && Equals(other);
            public override int GetHashCode() => BlockPtr.GetHashCode();
        }
    }
